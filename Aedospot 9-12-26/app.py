...
def init_db():
    ...
    # Persistent delay queue for Email/SMS notifications. A row is written
    # here the moment an alert (a risk_alerts row) is created — one row per
    # resident per enabled channel — with scheduled_for set to that
    # resident's OWN Notification Frequency setting (resident_alert.notif_freq)
    # added on top of the alert's creation time. The risk_monitor_loop thread
    # polls this table every tick and fires whatever has come due.
    #
    # Living in the database (not a Python dict in the monitor thread) is
    # what makes the delay survive an admin page refresh/close AND a full
    # server restart — the countdown was already computed and stored at
    # schedule time, so nothing needs to be re-armed on the next boot the way
    # the old in-memory timers had to be.
    #
    # The UNIQUE constraint stops the same alert from ever being scheduled
    # twice for the same resident/channel (e.g. a retried request), and the
    # atomic "claim" UPDATE the dispatcher uses (status: pending -> sending)
    # is what stops a row from ever being SENT twice.
    cursor.execute('''
    ...
    # Shared/admin-broadcast notifications (the `notifications` table above) are
    # visible to every resident in a barangay at once, so a resident deleting one
    # from their own Recent Alerts list must NOT delete the underlying row (that
    # would remove it for every other resident too). Instead we record that this
    # resident dismissed it, and the GET /api/notifications query below excludes
    # anything the current user has dismissed. risk_alerts rows don't need this —
    # they already have a user_id and are only ever visible to that one resident,
    # so those get hard-deleted directly.
    cursor.execute('''
    ...
    # Same idea as dismissed_notifications above, but for the ADMIN dashboard
    # (admin_alerts.html). risk_alerts/notifications rows are shared: a
    # resident's own Alerts page reads their risk_alerts rows straight from
    # this same table, so an admin hard-DELETEing from their dashboard used to
    # wipe those residents' alert history too. "Delete" on the admin side now
    # only records that the admin hid it here — the underlying row is left
    # completely alone, so residents keep seeing their own copy untouched.
    cursor.execute('''
    ...
    # Admin's GIS "Edit Trace" boundary edits (admin_risk_map.html), shared
    # with the read-only resident map (resident_risk_map.html). This used to
    # be saved to localStorage only, which is per-browser/per-device — an
    # admin's edits never reached a resident opening the map on their own
    # phone or computer even though the resident view is meant to be
    # view-only with the admin fully in control of the shapes. Storing the
    # saved GeoJSON here instead makes the database the single shared source
    # of truth: whichever admin last saved a boundary is what every resident
    # (and every other admin) sees. override_key is one of
    # 'barangay_boundaries' (the adjusted per-barangay polygons) or
    # 'manual_trace' (the freehand-drawn outline overlay).
    cursor.execute('''
    ...
    # notif_freq needs to hold EITHER a number (15/30/60) OR the literal string
    # 'off' — see update_notification_settings() and risk_monitor_loop() further
    # down, both of which already expect 'off' to be stored as-is (same reason
    # the sibling global_notif_freq column above is TEXT instead of INTEGER).
    # The ADD COLUMN above (or an older deploy, or a column added by hand in the
    # Supabase table editor) created this as INTEGER, which can't hold the
    # string 'off' and throws "invalid input syntax for type integer: 'off'"
    # the moment a resident sets their Notification Frequency to Turn Off.
    # Safe to run on every startup — converting an already-TEXT column to TEXT
    ...
    # sms_notif_freq is SMS's own independent counterpart to notif_freq above —
    # notif_freq now specifically means the EMAIL delay. Before this column
    # existed, SMS silently rode along on notif_freq (Email's setting), so a
    # resident with Email=30min/SMS=1hr had no way to actually get that: SMS
    # always fired whenever Email did. Same TEXT type/'off' handling as
    # notif_freq, and same reasoning for defaulting new/blank rows to '15'.
...
def get_environmental_data():
    ...
    try:
        with get_db_connection() as conn:
            ...
            cursor.execute('''
                SELECT reading_date, temperature, humidity, rainfall_mm, ph_level, wind_speed, risk_level
    ...
    while True:
        try:
            ...
            try:
                ...
                dev_cursor.execute('SELECT id FROM device_deployments')
                device_ids = [r[0] for r in dev_cursor.fetchall()]
                for did in device_ids:
                    ...
                    dev_cursor.execute('''
                        INSERT INTO device_daily_readings
                            (device_id, reading_date, temperature, humidity, rainfall_mm, ph_level, wind_speed, risk_level, updated_at)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
                        ON CONFLICT(device_id, reading_date) DO UPDATE SET
                            ...
                            updated_at = CURRENT_TIMESTAMP
                    ''', (did, today_str, temp, humidity, rain_mm, ph_level, wind_speed, current_risk))
                dev_conn.commit()
                dev_conn.close()
            except Exception as e:
